...
and k-fold cv for hyper parameter tuning.
...
| Reasonable computation time | Crazy computation time (low) | Depends on K            |

## Classification Problems

The label is not numeric but it is a member of a discrete set.
...
The features are: $XϵR^p$ . The feature that is categorical is converted
to numerical values through one-hot-encoding.
...
depends on the confidence of the results. This could be done through:
...
To see which measures are commonly used to evaluate the quality of the
model and perform model selection, the table below can be used:

![](images/paste-D2149627.png){width="584"}
...
The perfect prediction is recall = 1 & precision = 1. The bad prediction
is recall = 0 & precision = 0.

![](images/paste-C83C7277.png){width="427"}
...
We have to be careful with a unbalanced data set. This means that the
