...
two <- pnorm(X, mean = mean, sd = sd, lower.tail = FALSE)
three <- qnorm(p, mean = mean, sd = sd, lower.tail = FALSE) 
four <- diff(pnorm(c(X,Y), mean = mean, sd = sd, lower.tail = FALSE))
```
...
```{r}
data <- matrix(c(ToothGrowth$len), byrow=TRUE, nrow=2)  
