...
|               | '2021-12-31'     |
+---------------+------------------+
| Combine       | CONCAT(column1,  |
| multiple      | column2)         |
| columns       |                  |
| together      |                  |
+---------------+------------------+
| Combine       | CONCAT(column1,  |
| together with | '-', column2,    |
| individual    | ',', column3)    |
| separator     |                  |
+---------------+------------------+
| A function    | CONCAT_WS(',',   |
| that adds two | column1,         |
| or more       | column2)         |
| strings       |                  |
| together with |                  |
| a separator   |                  |
...
| The CASE      | CASE             |
| statement     |                  |
| goes through  | WHEN name =      |
| one or more   | 'wrong' THEN     |
| conditions    | 'Right'          |
| and returns a |                  |
| value as soon | ELSE name        |
| as a          |                  |
| condition is  | END AS           |
| met.          | cleaned_name     |
+---------------+------------------+
...
|                                | '2021-12-31'                        |
+--------------------------------+-------------------------------------+
| Transforming values /          | IF(                                 |
| characters                     |                                     |
|                                | columnname = amount or character,\  |
...
+--------------------------------+-------------------------------------+
| The **SAFE_CAST** function     | SAFE_CAST(expression AS typename)   |
| returns a value of Null        |                                     |
| instead of an error when a     |                                     |
| query fail                     |                                     |
+--------------------------------+-------------------------------------+
|                                |                                     |
+--------------------------------+-------------------------------------+
